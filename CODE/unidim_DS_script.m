% UNIDIM_DS_SCRIPT parameters scropt for MLC
% Type mlc=MLC('unidim_DS_script') to create corresponding MLC object
parameters.size = 50;
parameters.sensors = 1;
parameters.sensor_spec = 0;
parameters.sensor_list = 0;
parameters.controls = 1;
parameters.sensor_prob = 0.2;
parameters.leaf_prob = 0.3;
parameters.range = 10;
parameters.precision = 4;
parameters.opsetrange = [1 2 3 9];
parameters.formal = 0;
parameters.end_character = '';
parameters.maxdepth = 15;
parameters.maxdepthfirst = 8;
parameters.mindepth = 2;
parameters.mutmaxdepth = 15;
parameters.mutmindepth = 2;
parameters.mutsubtreemindepth = 2;
parameters.generation_method = 'mixed_ramped_gauss';
parameters.gaussigma = 3;
parameters.ramp = [2 3 4 5 6 7 8];
parameters.maxtries = 10;
parameters.mutation_types = [1 2 3 4];
parameters.elitism = 10;
parameters.probrep = 0.1;
parameters.probmut = 0.4;
parameters.probcro = 0.5;
parameters.selectionmethod = 'tournament';
parameters.tournamentsize = 7;
parameters.lookforduplicates = 1;
parameters.simplify = 0;
parameters.evaluation_method = 'standalone_function';
parameters.evaluation_function = 'unidim_DS_evaluator';
parameters.indfile = 'ind.dat';
parameters.Jfile = 'J.dat';
parameters.exchangedir = '/media/homes/thomas/00_Git_repos/OpenMLC-Matlab/evaluator0';
parameters.evaluate_all = 0;
parameters.ev_again_best = 0;
parameters.ev_again_nb = 5;
parameters.ev_again_times = 5;
parameters.artificialnoise = 0;
parameters.execute_before_evaluation = '';
parameters.badvalue = 1e+36;
parameters.badvalues_elim = 'first';
parameters.preevaluation = 0;
parameters.preev_function = '';
parameters.save = 1;
parameters.saveincomplete = 1;
parameters.verbose = 2;
parameters.fgen = 250;
parameters.show_best = 1;
parameters.problem_variables.Tf = 5;
parameters.problem_variables.objective=1;
%parameters.problem_variables.objective1=0;
%parameters.problem_variables.objective2=100;
parameters.problem_variables.gamma=0.1;
parameters.problem_variables.Tevmax=1;

parameters.problem_variables.N=6; %or N=12 to 256,
parameters.problem_variables.G=digraph([1 2 2 3 4 5 6 6 6],[6 4 6 5 1 2 2 3 4],[10 5 5 10 9 9 7 2 2],{'1','2','3','4','5','6'});
parameters.problem_variables.A=adjacency(digraph([1 2 2 3 4 5 6 6 6],[6 4 6 5 1 2 2 3 4],[10 5 5 10 9 9 7 2 2],{'1','2','3','4','5','6'}),'weighted');
parameters.problem_variables.omega=ones(6,1)*1/100; %or ones(12,1)*1/100 to ones(256,1)*1/100;;
parameters.problem_variables.B=adjacency(digraph([1 2 2 3 4 5 6 6 6],[6 4 6 5 1 2 2 3 4],[10 5 5 10 9 9 7 2 2],{'1','2','3','4','5','6'}));
parameters.problem_variables.kappa=0.001;
%parameters.problem_variables.A=[ones(12/2) ones(12/2);ones(12/2) ones(12/2)] - eye(12);



parameters.savedir = '/media/homes/thomas/00_Git_repos/OpenMLC-Matlab/save_GP/20180321-1518';
