%% Code for the weighted network of 6 nodes of Kuramoto oscillators with Kuramoto equation solutions showing simulations of the network having control input to ensure the synchronization of desynchronized oscillators.
...
dt = 0.01;
max_step = 300/dt; %1000/dt

%% parameters
omega = ones(N,1)*1/100; % natural frequency parameters

%% initialization
theta = zeros (N, max_step); % phase initial state
...
for t = 1:max_step
    ...
    K = rand(N,N); %% control input
    %K = zeros(N,N); %% zero control input to check without control mechanism
    F = A.*K;
   ...
   for i = 1:N
        coup (i) = 0.001*sum(A(i,:)'.* sin(theta(:,t) - theta(i,t)));
        control (i) = sum(F(i,:)'.* sin(theta(:,t) - theta(i,t)));
        dthetadt (i) = omega(i) + 1/N*coup (i) + 1/N*control (i);
